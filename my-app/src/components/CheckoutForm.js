  ...
  const placeOrder = async () => {
    ...
    const response = await fetch("http://localhost:3000/api/orders/place", {
      ...
      body: JSON.stringify({
        ...
        shippingDetails: { email, contactNumber, address, city, zip, state: stateName, postalCode, country },
      }),
    });
  ...
  return (
    <div className="checkout-container">
      ...
      <div className="order-summary">
        ...
        {items.map((item, idx) => (
          <p key={idx}>
            {item.name || "No Name"} (x{item.quantity || 1}) - ₹{(item.price || 0).toLocaleString()}
          </p>
        ))}
